async function getPokemon() {
  ...
  for (
    var i = 0;
    i < response2.data.damage_relations.double_damage_to.length;
    i++
  ) {
    var type =
      response2.data.damage_relations.double_damage_to[i].name
        .charAt(0)
        .toUpperCase() +
      response2.data.damage_relations.double_damage_to[i].name.slice(1);
  ...
  for (
    var i = 0;
    i < response2.data.damage_relations.double_damage_from.length;
    i++
  ) {
    var type =
      response2.data.damage_relations.double_damage_from[i].name
        .charAt(0)
        .toUpperCase() +
      response2.data.damage_relations.double_damage_from[i].name.slice(1);
    document.getElementById("weak").innerHTML += type + "<br>";
  }

  if (types.length > 1) {
    const response3 = await axios.get(
      `https://pokeapi.co/api/v2/type/${response.data.types[1].type.name}`
    );
    for (
      var i = 0;
      i < response3.data.damage_relations.double_damage_to.length;
      i++
    ) {
      var type =
        response3.data.damage_relations.double_damage_to[i].name
          .charAt(0)
          .toUpperCase() +
        response3.data.damage_relations.double_damage_to[i].name.slice(1);
      document.getElementById("strong").innerHTML += type + "<br>";
    }

    for (
      var i = 0;
      i < response3.data.damage_relations.double_damage_from.length;
      i++
    ) {
      var type =
        response3.data.damage_relations.double_damage_from[i].name
          .charAt(0)
          .toUpperCase() +
        response3.data.damage_relations.double_damage_from[i].name.slice(1);
      document.getElementById("weak").innerHTML += type + "<br>";
    }
  }
